/*
    Create an LMS Equalizer for a dispersive communication channel.  Choose the
    square wave frequency.  Plot the impulse and frequency responses
	of the filter.  Filter noisy signal using the equalizer.
...
const (
	...
	tmplLmsEqualizer    = "templates/LMSequalizer.html"      // html template address
	tmplfiltersignal    = "templates/filtersignal.html"      // html template address
	addr                = "127.0.0.1:8080"                   // http server listen address
	patternFilterSignal = "/filtersignal"                    // http handler pattern for filtering using the LMS Equalizer
	patternLmsEqualizer = "/lmsequalizer"                    // http handler pattern for creating LMS equalizer
	...
	lmsFilterEqualizer  = "lmsequalizer.txt"                 // LMS adaptive filter for Equalizer
)
...
type PlotT struct {
	...
	SigFreq     string   // square wave frequency in Hz
...
type ComChan struct {
	desired float64 // delayed source signal to the LMS algorithm
	in      float64 // input to the stage in the pipeline
}

// Data to construct the processing chain for display
type FilterSignal struct {
	toChan      chan float64 // synchronized channel to dispersive channel from signal source generator
	fromChan    chan float64 // synchronized channel to adaptive filter from dispersive channel
	wg          sync.WaitGroup
...
var (
	...
	lmsequalizerTmpl *template.Template
...
func init() {
	...
	lmsequalizerTmpl = template.Must(template.ParseFiles(tmplLmsEqualizer))
...
func (lms *LMSAlgorithm) generateSignal() error {
	...
	// a pseudorandom square wave at +/- 1
	go func() {
		defer func() {
			lms.wg.Done()
			close(lms.toChan)
...
// chanFilter filters the signal and adds Gaussian noise at the SNR
func (lms *LMSAlgorithm) chanFilter() error {
	...
	go func() {
		...
		noiseSD := math.Sqrt(math.Pow(10.0, -float64(lms.snr)/10.0))

		// range over the input channel
		for d := range lms.toChan {
			temp := d.in + noiseSD*rand.NormFloat64()
			for i, val := range r {
				temp += h[i] * val
			}

			// Send output to next stage via the output channel
			lms.fromChan <- ComChan{desired: d.desired, in: temp}
...
// handleLmsEqualizer creates an Equalizer using the LMS algorithm
func handleLmsEqualizer(w http.ResponseWriter, r *http.Request) {
	var plot PlotT
	...
	if len(samplestxt) > 0 && len(sampleratetxt) > 0 {
		...
		for i := 0; i < lmsEqualizer.trials; i++ {

			// Create new channels each trial since they are closed after each trial
			lmsEqualizer.fromChan = make(chan ComChan)
			lmsEqualizer.toChan = make(chan ComChan)

			// Generate a new set of input data consisting of a pseudorandom square wave +/- 1
			err = lmsEqualizer.generateSignal()
			if err != nil {
			...
			// Run the signal through the dispersive channel with added Gaussian noise
			err = lmsEqualizer.chanFilter()
			if err != nil {
			...
			err = lmsEqualizer.runLms()
			if err != nil {
				plot.Status = fmt.Sprintf("generateLMSData error: %v", err.Error())
				fmt.Printf("runLmsEqualizer error: %v\n", err.Error())
				// Write to HTTP using template and grid
				if err := lmsequalizerTmpl.Execute(w, plot); err != nil {
					log.Fatalf("Write to HTTP output using template with error: %v\n", err)
				}
			...
			lmsEqualizer.wg.Wait()
		}

		// Save ensemble filter coefficients to disk (averaged coefficients)
		file, err := os.Create(path.Join(dataDir, lmsFilterEqualizer))
		if err != nil {
			plot.Status = fmt.Sprintf("create %v error: %v", path.Join(dataDir, lmsFilterEqualizer), err.Error())
			fmt.Printf("create %v error: %v\n", path.Join(dataDir, lmsFilterEqualizer), err.Error())
			// Write to HTTP using template and grid
			if err := lmsequalizerTmpl.Execute(w, plot); err != nil {
				log.Fatalf("Write to HTTP output using template with error: %v\n", err)
			}
...
// generateSignal creates pseudo-random square wave at +/- 1
func (fs *FilterSignal) generateSignal() error {
	...
	if fs.display == "channelin" {
		...
		go func() {
			...
			// Send a signal sample to dispersive channel via the toChannel
			for i := 0; i < fs.samples; i++ {
				// new sample, randomly chosen
				if i%r == 0 {
					if rand.Float64() < 0.5 {
...
// equalizerFilter filters the signal with the LMS equalizer
func (fs *FilterSignal) equalizerFilter() error {
	// determine where to send the signal
...
func (fs *FilterSignal) labelExec(w http.ResponseWriter, plot *PlotT) {
	...
	if len(plot.Status) == 0 {
		plot.Status = fmt.Sprintf("Signal consisting of (+/-)1 square wave in dispersive channel"+
...
// handleFilter filters a noisy signal using the LMS Equalizer
func handleFilterSignal(w http.ResponseWriter, r *http.Request) {
	...
	if len(temp) > 0 {
		...
		if display == "channelout" || display == "equalizerout" {
			txt := r.FormValue("snr")
			snr, err = strconv.Atoi(txt)
			if err != nil {
				plot.Status = fmt.Sprintf("SNR conversion to int error: %v", err.Error())
		...
		// Generate input data consisting of a pseudo-random square wave +/- 1
		...
		// Run the signal through the dispersive channel with white Gaussian noise
		err = fs.chanFilter()
		if err != nil {
			plot.Status = fmt.Sprintf("chanFilter error: %v", err.Error())
			fmt.Printf("chanFilter error: %v\n", err.Error())
		...
		// Run the dispersive channel output through the adaptive Equalizer
		err = fs.equalizerFilter()
		if err != nil {
			plot.Status = fmt.Sprintf("equalizerFilter error: %v", err.Error())
			fmt.Printf("equalizerFilter error: %v\n", err.Error())
			// Write to HTTP using template and grid
			if err := filterSignalTmpl.Execute(w, plot); err != nil {
				log.Fatalf("Write to HTTP output using template with error: %v\n", err)
			}
...
func main() {
	...
	// Setup http server with handler for filtering a noisy signal using the LMS Equalizer
	http.HandleFunc(patternFilterSignal, handleFilterSignal)

	// Setup http server with handler for creating an Equalizer using LMS
	http.HandleFunc(patternLmsEqualizer, handleLmsEqualizer)

	// Setup http server with handler for plotting impulse or frequency responses for the LMS Equalizer
	http.HandleFunc(patternPlotResponse, handlePlotResponse)

	fmt.Printf("LMS Equalizer Server listening on %v.\n", addr)
